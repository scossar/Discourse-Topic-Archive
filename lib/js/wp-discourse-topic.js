jQuery(document).ready(function ($) {
    ...
    function load_discourse_topic() {
        ...
        var $discourse_url = $('#discourse-url'),
            url = $discourse_url.val(), // The value that has been entered.
            absolute_url,
            base_url,
            initial_request_url,
            post_request_url;
        ...
        if (!url) {
            ...
            $.getJSON(ajaxurl, data, function (response) {
                ...
                var chunk_size, // The number of posts per request (defaults to 20 in Discourse)
                    stream,
                    $target = $('#content'); // This is where we are going to output the topic posts.
                ...
                assemble_content(response, stream, [], chunk_size);

            }).fail(function (response) {
                $('.loading').removeClass('discourse-loading');
                warning('There was no response returned from the server. ' +
                    'Please make sure you are providing the URL to a Discourse topic ' +
                    'and that the Discourse forum you are trying to connect to is online.' );
            });
        }

        // Returns an array of posts.
        // Calls itself recursively while there are still post_ids in the post stream.
        function assemble_content(response, post_stream, buffer, chunk_size) {
            ...
            if (next_request_ids.length) {
                ...
                $.getJSON(ajaxurl, data, function (response) {
                    add_meta_box_post_content(response, post_stream, buffer, chunk_size);
                });
    ...
    function add_posts_to_editor() {
        var selected_posts = [],
            joined_posts;

        $.each($('.post-select'), function () {
            if ($(this).find('.post-select-box').prop('checked')) {
                selected_posts.push($(this).find('.topic-post').html());
            }
        });

        joined_posts = selected_posts.join('');

        $('#content').html(joined_posts);
    }
